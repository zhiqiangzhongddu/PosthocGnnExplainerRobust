...
from utils import zoom_function
EPS = 1e-15
...
# class RobustGNNExplainer(ExplainerBase):
#     r"""The GNN-Explainer model from the `"GNNExplainer: Generating
#     Explanations for Graph Neural Networks"
#     <https://arxiv.org/abs/1903.03894>`_ paper for identifying compact subgraph
#     structures and small subsets node features that play a crucial role in a
...
#                  explain_graph: bool = False,
#                  indirect_graph_symmetric_weights: bool = False):
#         super(RobustGNNExplainer, self).__init__(model, epochs, lr, explain_graph)
#         self.coff_node_feat_size = coff_node_feat_size
#         self.coff_node_feat_ent = coff_node_feat_ent
#         self.coff_edge_size = coff_edge_size
#         self.coff_edge_ent = coff_edge_ent
...
#             # my edit
#             if self.mode == 1:
#                 raw_preds = F.sigmoid(raw_preds)
#             elif self.mode == 2:
#                 raw_preds = F.softmax(raw_preds, dim=-1)
#                 raw_preds = zoom_function(x=raw_preds, a=10, b=0.5)
#             loss = self.__loss__(raw_preds, ex_label)
#             if epoch % 20 == 0 and debug:
#                 print(f'Loss:{loss.item()}')
#
